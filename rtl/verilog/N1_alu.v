...
module N1_alu
  #(localparam CELL_WIDTH    = 16,                           //cell width
   ...
   (//IR interface
    ...
    input  wire [CELL_WIDTH:0]       us_ps0_i,               //PS0 (TOS)
    input  wire [CELL_WIDTH:0]       us_ps1_i,               //PS1 (TOS+1)
   ...
   //Bitwise logic
   wire [CELL_WIDTH-1:0]     bl_op0;                         //first operand
   wire [CELL_WIDTH-1:0]     bl_and;                         //result of AND operation
   wire [CELL_WIDTH-1:0]     bl_xor;                         //result of XOR operation
   wire [CELL_WIDTH-1:0]     bl_or;                          //result of OR operation
   wire [(2*CELL_WIDTH)-1:0] bl_out;                         //bit logic output
   //Barrel shifter
   wire [CELL_WIDTH-1:0]     lsr_op0;                        //first operand
   wire [CELL_WIDTH-1:0]     lsr_op1;                        //second operand
   wire                      lsr_msb;                        //MSB of first operand
   wire [CELL_WIDTH-1:0]     lsr_sh0;                        //shift 1 bit
   wire [CELL_WIDTH-1:0]     lsr_sh1;                        //shift 2 bits
   wire [CELL_WIDTH-1:0]     lsr_sh2;                        //shift 4 bits
   wire [CELL_WIDTH-1:0]     lsr_sh3;                        //shift 8 bits
   wire [CELL_WIDTH-1:0]     lsr_sh4;                        //shift 16 bits
   wire [(2*CELL_WIDTH)-1:0] lsr_res;                        //result
   wire [CELL_WIDTH-1:0]     lsl_op0;                        //first operand
   wire [CELL_WIDTH-1:0]     lsl_op1;                        //second operand
   wire [(2*CELL_WIDTH)-1:0] lsl_sh0;                        //shift 1 bit
   wire [(2*CELL_WIDTH)-1:0] lsl_sh1;                        //shift 2 bits
   wire [(2*CELL_WIDTH)-1:0] lsl_sh2;                        //shift 4 bits
   wire [(2*CELL_WIDTH)-1:0] lsl_sh3;                        //shift 8 bits
   wire [(2*CELL_WIDTH)-1:0] lsl_sh3;                        //shift 16 bits
   wire [(2*CELL_WIDTH)-1:0] lsl_sh4;                        //shift 32 bits
   wire [(2*CELL_WIDTH)-1:0] lsl_res;                        //result
   wire [(2*CELL_WIDTH)-1:0] ls_out;                         //shifter output
   //Literal value
   wire [(2*CELL_WIDTH)-1:0] lit_out;                         //literal value output
   //Processor status
   wire [(2*CELL_WIDTH)-1:0] stat_out;                        //processor status output
   ...
   //Result
   assign cmp_out         = {2*CELL_WIDTH{cmp_muxed}};
   ...
   assign bl_and           = ~|{ir_alu_opr_i[1:0]^2'b00} ?
                             (imm_is_zero ? (us_ps0_i & us_ps1_i) : (us_ps0_i & simm)) : {CELL_WIDTH{1'b0}};
   //XOR
   assign bl_xor            = ~(ir_alu_opr_i[0]^1'b0) ?
                             (imm_is_zero ? (us_ps0_i ^ us_ps1_i) : (us_ps0_i ^ simm)) : {CELL_WIDTH{1'b0}};
   //AND
   assign bl_or            = ~|{ir_alu_opr_i[1:0]^2'b10} ?
                             (imm_is_zero ? (us_ps0_i | us_ps1_i) : (us_ps0_i | uimm)) : {CELL_WIDTH{1'b0}};
   //Result
   assign bl_out           = ~|{ir_alu_opr_i[4:2]^3'b100} ?
                              {{CELL_WIDTH{1'b0}}, bl_and | bl_xor | bl_or} : {2*CELL_WIDTH{1'b0}};
   ...
   assign lsr_msb = opr_asr ? lsr_op0[CELL_WIDTH] : 1'b0;    //MSB of first operand
   assign lsr_sh0 = lsr_op1[0] ?                             //shift 1 bit
                    {lsr_msb, lsr_op0[CELL_WIDTH-1:1]} :     //
                    lsr_op0;                                 //
   assign lsr_sh1 = lsr_op1[1] ?                             //shift 2 bits
                    {{2{lsr_msb}}, lsr_sh0[CELL_WIDTH-1:2]} ://
                    lsr_sh0;                                 //
   assign lsr_sh2 = lsr_op1[2] ?                             //shift 4 bits
                    {{4{lsr_msb}}, lsr_sh1[CELL_WIDTH-1:4]} ://
                    lsr_sh1;                                 //
   assign lsr_sh3 = lsr_op1[3] ?                             //shift 8 bits
                    {{8{lsr_msb}}, lsr_sh2[CELL_WIDTH-1:8]} ://
                    lsr_sh2;                                 //
   assign lsr_sh4 = |lsr_op1[CELL_WIDTH-1:4] ?               //shift 16 bits
                    {16{lsr_msb}} :                          //
                    lsr_sh3;                                 //
   assign lsr_res = ~ir_alu_opr_i[0] ?                       //result
                    {{CELL_WIDTH{lsr_msb}},clsr_sh4} :       //
                    {2*CELL_WIDTH{1'b0}};                    //
   //Left shift
   ...
   assign lsl_sh0 = lsl_op1[0] ?                             //shift 1 bit
                    {{CELL_WIDTH-1{1'b0}}, lsl_op0, 1'b0} :  //
                    {{CELL_WIDTH{1'b0}}, lsl_op0};           //
   assign lsl_sh1 = lsl_op1[1] ?                             //shift 2 bits
                    {lsl_sh0[(2*CELL_WIDTH)-3:0], 2'b0} :    //
                    lsl_sh0;                                 //
   assign lsl_sh2 = lsl_op1[2] ?                             //shift 4 bits
                    {lsl_sh0[(2*CELL_WIDTH)-5:0], 4'h0} :    //
                    lsl_sh1;                                 //
   assign lsl_sh3 = lsl_op1[3] ?                             //shift 8 bits
                    {lsl_sh0[(2*CELL_WIDTH)-9:0], 8'h00} :   //
                    lsl_sh2;                                 //
   assign lsl_sh3 = lsl_op1[4] ?                             //shift 16 bits
                    {lsl_sh0[(2*CELL_WIDTH)-16:0], 16'h0000}://
                    lsl_sh3;                                 //
   assign lsl_sh4 = |lsl_op1[CELL_WIDTH-1:5] ?               //shift 32 bits
                    {2*CELL_WIDTH{1'b0}}:                    //
                    lsl_sh3;                                 //
   assign lsl_res = ir_alu_opr_i[0] ?                        //shift 32 bits
                    lsl_sh4 : {2*CELL_WIDTH{1'b0}};          //
   //Result
   assign ls_out  = ~|{ir_alu_opr_i[4:2]^3'b101} ?
                    (lsr_res | lsl_res) : {2*CELL_WIDTH{1'b0}};

   //Literal value
   //-------------
   assign lit_out = ~|{ir_alu_opr_i[4:2]^3'b110} ?
                    {{CELL_WIDTH-1{simm[5]}}, simm, us_ps0_i[11:0} :
                    {2*CELL_WIDTH{1'b0}};

   //Processor status
   //----------------
   assign stat_out = ~|{ir_alu_opr_i[4:2]^3'b111} ?
                     (({2*CELL_WIDTH{~|{ir_alu_opr_i[1:2]^2'b00}}} & cpu_stat_i)                         |
                      ({2*CELL_WIDTH{~|{ir_alu_opr_i[1:2]^2'b01}}} & excpt_throw_code_i)                 |
                      ({2*CELL_WIDTH{~|{ir_alu_opr_i[1:2]^2'b10}}} & {lps_sp_i, ips_stat_i, us_pstat_i}) |
                      ({2*CELL_WIDTH{~|{ir_alu_opr_i[1:2]^2'b11}}} & {lrs_sp_i, irs_stat_i, us_rstat_i})) :
                     {2*CELL_WIDTH{1'b0}};
