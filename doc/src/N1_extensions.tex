...
The \gls{rotext} adds two data paths to the \gls{us}, allowing direct dater transfers
between the top and the third element of the \gls{ps}. These new stack transitions are
are perfomed by the regular stack instructions (see \secref{opcodes:stack}), using
...
(see tabref{extensions:rot:mapping}). 
That means that all common single-cell operations \gls{ps} operations shown in
...
None of the stack operations listed in \tabref{opcodes:stack:mapping} are affected by
the \gls{rotext}.
