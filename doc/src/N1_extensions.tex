...
%EKEY/EMIT extension
\subsection{EKEY/EMIT Extension}
\label{extensions:ekey}

The EKEY/EMIT extension adds support for an I/O device to the N1 core.
\gls{forth} words EKEY, EKEY?, EMIT, ands EMIT? dre direcly mapped to a set of \gls{freg}.

%EKEY? register
\subsubsection{EKEY? Register}
\label{extensions:ekey:ekeyq}

The EKEY? register (see \figref{extensions:ekey:ekeyq:fig}) indicates whether there is data to be received from the input device. 
  ...
  \makebox[\textwidth][c]{
    ...
    \scalebox{0.515} {
      \begin{tikzpicture}
        ...
        \node at (16,1.5)   {\Large{\texttt{EKEY?}}};
        \end{tikzpicture}
    }
  }
  \caption{EKEY? Register}
  \label{extensions:ekey:ekeyq:fig}
  \end{center}
\end{figure}


The \texttt{EKEY?} field in this read only register resembles the functionality of the \texttt{EKEY?} word
(see \tabref{extensions:ekey:ekeyq:tab}.
...
The EMIT? (see \figref{extensions:ekey:emitq:fig}) register indicates whether the I/O device is ready to transmit data. 
...
The PEEK register (see \figref{extensions:ekey:peek:fig}) allows the software to preview at the input buffer of the I/O device,
...
\begin{figure}[H]
  ...
  \caption{EMIT? Register}
  \label{extensions:ekey:peek:fig}
  \end{center}
\end{figure}

The data in the \texttt{PEEK} field (see \tabref{extensions:ekey:emitq:tab} is only valid if
\texttt{EKEY?} holds a true value.
...
The EKEY/EMIT register (see \figref{extensions:ekey:ekey:fig} is used to receive data from and to transmit daza to the I/O device.
...
\begin{figure}[H]
  ...
  \caption{EKEY/EMIT Register}
  \label{extensions:ekey:ekey:fig}
  \end{center}
\end{figure}

The \texttt{EKEY} field resembles the \texttt{EKEY} word and the \texttt{EMIT} field resembles the \texttt{EMIT} word
(see \tabref{extensions:ekey:ekey:tab}), 
...
\begin{center}
  ...
  \begin{longtable}{|c|c|c|c|}
    \rowcolor{white}
    \caption{Exception and Interrupt Mask Register Bit Description}
    \label{extensions:ekey:ekey:tab} \\
    %Header
    \hline                                     
    \rowcolor{gray!25}
    \multicolumn{1}{|c|}{\textbf{\rule{0pt}{2.5ex}Bit}}       &  
