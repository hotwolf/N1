...
\item[Memory connection:]  
  ...
  \item Memory addressable in 16-bit entities 
  \end{itemize}
...
\item[Stacks:]
  ...
  \item Each \gls{stack} consists of three segments:
    ...
    \item[\Gls{us}:]
      \begin{itemize}
      \item[]
      \item Shift registers with selectable shift direction for each entry
      \item Fixed size
        \begin{itemize}
        \item \gls{ps}: 4 \glspl{cell}
        \item \gls{rs}: 1 \gls{cell}
        \end{itemize}
      \end{itemize}
    %Intermediate stack  
    \item[\Gls{is}:]
      \begin{itemize}
      \item[]
      \item \Gls{lifo} buffer with lazy data transfers to and from the lower stack
