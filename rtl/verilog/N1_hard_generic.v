...
module N1_hard
  #(parameter   SP_WIDTH   =  8,                               //width of a stack pointer
   ...
   //---
   //Adder
   assign alu_sum  = {CELL_WIDTH+1{alu_sub_i}} ^
                     (({CELL_WIDTH{alu_sub_i}} ^ alu_op1_i) + alu_op0_i);

   //Multiplier
   assign alu_prod = {{CELL_WIDTH{alu_smul_i & alu_op0_i[CELL_WIDTH-1]}}, alu_op0_i} *
                     {{CELL_WIDTH{alu_smul_i & alu_op1_i[CELL_WIDTH-1]}}, alu_op1_i};

   //Output
   assign alu_o = ({2*CELL_WIDTH{alu_add_i  | alu_sub_i}}  & {{CELL_WIDTH-1{alu_sum[CELL_WIDTH]}},alu_sum}) |
                  ({2*CELL_WIDTH{alu_umul_i | alu_smul_i}} & alu_prod[(2*CELL_WIDTH)-1:0]);
