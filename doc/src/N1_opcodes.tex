...
\texttt{IEN} (interrupt enable) and \texttt{EEN} (exception enable)
(see \figref{opcodes:ctrl:irq:fig} and \tabref{opcodes:ctrl:irq:tab}).
...
Each one of these bits is protected by a mask bit to atomic bit manipulations.
...
(see \figref{opcodes:ctrl:psd:fig} and \tabref{opcodes:ctrl:psd:fig}). \\
...
(see \figref{opcodes:ctrl:rsd:fig} and \tabref{opcodes:ctrl:rsd:fig}). \\
...
Immediate address offset register (see \figref{opcodes:ctrl:iaoffs:fig} and \tabref{opcodes:ctrl:iaoffs:tab})
provides an address offset fore the immediate address mode. \\
The effectife address is the sum of immediate address and immediate address offset.
    ...
    \scalebox{0.515} {
      \begin{tikzpicture}
       ...
       %\node at  (8,1.5) {\huge{\texttt{IAOFFS}}};
        ...
        \node [align=right, anchor=east] at (0,0.5) {\large{Write}};
        \draw [thick, fill=white]   (0,0) rectangle (32,1);
        \draw [thick, fill=gray!3]  (0,0) rectangle (16,2); %15..8
        \node at  (8,1)   {\huge{\texttt{IAOFFS}}};
      \end{tikzpicture}
    }
  }
