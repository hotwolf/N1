...
//###############################################################################
//# Description:                                                                #
//#    This is the top level of the N1's synthesizable partition.               #
//#                                                                             #
//###############################################################################
//# Version History:                                                            #
//#   January 10, 2019                                                          #
...
module N1_soft
  #(parameter   RST_ADR    = 15'h0000,                         //address of first instruction
   ...
   (//Clock and reset
    ...
    output wire                             lps_agu_psh_o,     //push (decrement address)
    output wire                             lps_agu_pul_o,     //pull (increment address)
    output wire                             lps_agu_rst_o,     //reset AGU
    input  wire [SP_WIDTH-1:0]              lps_agu_i,         //result
    //Lower return stack AGU (stack grows higher addresses)
    output wire                             lrs_agu_psh_o,     //push (increment address)
    output wire                             lrs_agu_pul_o,     //pull (decrement address)
    output wire                             lrs_agu_rst_o,     //reset AGU
    input  wire [SP_WIDTH-1:0]              lrs_agu_i,         //result
    //Program Counter
    output wire                             pc_rel_o,          //add address offset
    output wire                             pc_abs_o,          //drive absolute address
    output wire                             pc_update_o,       //update PC
    input  wire [PC_WIDTH-1:0]              pc_rel_adr_o,      //relative COF address
    input  wire [PC_WIDTH-1:0]              pc_abs_adr_o,      //absolute COF address
    input  wire [PC_WIDTH-1:0]              pc_next_i,         //result
    //ALU
    ...
    input  wire [(2*CELL_WIDTH)-1:0]        alu_i,             //result
   ...
   //Probes from hard IP
   //-------------------
   assign prb_lps_sp_o  = lps_agu_i;                         //lower parameter stack pointer
   assign prb_lrs_sp_o  = lrs_agu_i;                         //lower return stack pointer
   assign prb_pc_next_o = pc_next_i;                         //next program counter
   ...
   N1_is
      #(.SP_WIDTH        (SP_WIDTH),                          //width of a stack pointer
        .CELL_WIDTH      (CELL_WIDTH),                        //cell width
        .IS_DEPTH        (IS_DEPTH),                          //depth of the intermediate stack
        .LS_GROW_UPWARDS (1))                                 //grow lower stack towards lower addresses
   N1_irs				                     
     (//Clock and reset			                     
      .clk_i		(clk_i),                             //module clock
      .async_rst_i	(async_rst_i),                       //asynchronous reset
