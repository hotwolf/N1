...
A ``return from call'' (\texttt{;}) is only executed if the branch is not taken.
...
\begin{center}
  ...
  \begin{longtable}{|c|c|c|}
    ...
    %Throw Code Register (TC)
    \texttt{0x02}                       &
    \hyperref[extensions:int]{TC}       &
    \hyperref[extensions:int]{Throw Code Register} \\ \hline

    %%Program Counter (PC)
    \texttt{0x03}                       &
    \hyperref[extensions:pc]{PC}        &
    \hyperref[extensions:pc]{\textbf{Optional} Program Counter Register} \\ \hline

    %KEY? Register (KEY?)
    \texttt{0x04}                       &
    \hyperref[extensions:key]{KEY?}     &
    \hyperref[extensions:key:keyq]{\textbf{Optional} KEY? Register} \\ \hline

    %EMIT? Register (EMIT?)
    \texttt{0x05}                       &
    \hyperref[extensions:key]{EMIT?}    &
    \hyperref[extensions:key:emitq]{\textbf{Optional} EMIT? Register} \\ \hline

%   %PEEK Register (PEEK)
%   \texttt{0x06}                        &
%   \hyperref[extensions:ke]{PEEK}       &
%   \hyperref[extensions:ke]{\textbf{Optional} PEEK Register} \\ \hline

    %KEY/EMIT Register (KEY)
    \texttt{0x06}                       &
    \hyperref[extensions:key]{KEY}      &
    \hyperref[extensions:key:key]{\textbf{Optional} KEY/EMIT Register} \\ \hline
...
%Throw Code Register (TC)
\subsubsection{Throw Code Register (TC)}
\label{opcodes:freg:tc}
  ...
  \makebox[\textwidth][c]{
    ...
    \scalebox{0.515} {
      \begin{tikzpicture}
        ...
        %\draw [thick, fill=white]  (0,1) rectangle (32,2);
        \draw [thick, fill=gray!3]  (0,1) rectangle  (32,2); %15..0
        \node at (15,1.5) {\huge{\texttt{TC}}};
        %Write
        \node [align=right, anchor=east] at (0,0.5) {\large{Write}};
        %\draw [thick, fill=white]  (0,0) rectangle (32,1);
        \draw [thick, fill=gray!3]  (0,0) rectangle  (32,1); %15..0
        \node at (15,0.5) {\huge{\texttt{THROW}}};
      \end{tikzpicture}
    }
  }
  %\caption{Throw Code Register}
  %\label{opcodes:freg:tcr:fig}
  %\end{center}
\end{figure}

The Throw Code Register (TC) captures the \gls{tc} of the last exception (see \tabref{reset:tc}).
\Gls{tc} values \texttt{0x0000} and \texttt{0x0001} indicate, that there is no pending interrupt or exception (see \tabref{reset:tc}).
Writing any value other than \texttt{0x0000} and \texttt{0x0001} to this register will trigger an exception (see \secref{reset:excpt}).
With the help of the \hyperref[extensions:int]{Interrupt Extension}, exceptions can also be triggered by external hardware events.
...
\begin{center}
  ...
  \begin{longtable}{|c|c|c|c|}
    \rowcolor{white}
    \caption{Throw Code Register Bit Description}
    \label{opcodes:freg:tcr:tab} \\
    %Header
    \hline                                     
    \rowcolor{gray!25}
    \multicolumn{1}{|c|}{\textbf{\rule{0pt}{2.5ex}Bit}}  &  
    ...
    \multicolumn{1}{m{36em}|}{
      \makecell[l]{
        \begin{minipage}[t]{\linewidth}%  
          ...
          \begin{description}[style=nextline]
          \item[Read:]
            \gls{tc} of the last exception.\\[1pt]
          \end{description}
        \end{minipage}%
    }}  \\ \hline
...
%\subsubsection{Interrupt Mask Register (IM)}
%\label{opcodes:freg:im}
%
%\begin{figure}[H]
%  %\begin{center}
%  \makebox[\textwidth][c]{
...
%\end{figure}
%
%Interrupt  handling can be disabled through setting the interrupt mask bit \texttt{IM}.
%
%The \texttt{IM} bit is automatically set when an interrupt service routine is started
%and must be cleared manually by writing a zero to the Interrupt Mask Register (see \tabref{opcodes:freg:eim:tab}).
%
%\begingroup
%\setlength{\LTleft}{-20cm plus -1fill}
%\setlength{\LTright}{\LTleft}
