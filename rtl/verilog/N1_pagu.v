...
module N1_pagu
  #(parameter PBUS_AADR_OFFSET = 16'h0000,                       //offset for direct program address
   ...
   //-------------------
   //Direct addresses
   wire [15:0]                       full_aadr = {PBUS_AADR_OFFSET[15:14], ir2pagu_aadr_i};
   wire [15:0]                       full_radr = {{3{ir2pagu_radr_i[12]}}, ir2pagu_radr_i};
   wire [15:0]                       full_madr = {PBUS_MADR_OFFSET[15:8],  ir2pagu_madr_i};
   //Address register
   ...
   always @(posedge async_rst_i or posedge clk_i)
     begin
        if (async_rst_i)                                         //asynchronous reset
          ...
          prev_adr_reg <= 16'h0000;                              //reset value
        else if (~fc2pagu_prev_adr_hold_i)                       //update address register
          prev_adr_reg <= dsp2pagu_adr_i;                        //AGU output
     end // always @ (posedge async_rst_i or posedge clk_i)

   //PBUS address
   assign pbus_adr_o = fc2pagu_prev_adr_sel_i ? dsp2pagu_adr_i : //AGU output
                                                prev_adr_reg;    //address register

   //Stack input
   assign pagu2prs_prev_adr_o = prev_adr_reg;                    //address register

   //Probe signal
   assign prb_pagu_prev_adr_o = prev_adr_reg;                    //address register
